        ...
        for (int i = 0; i < 6; i++)
        {
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    Eigen::Vector3f dir = cubemapDirs[i * width * height + y * width + x];  // get dir for the target pixel on cubemap
                    int index = (y * width + x) * channel;
                    Eigen::Array3f Le(images[i][index + 0], images[i][index + 1],images[i][index + 2]);

                    float delta_PHI = CalcArea(x, y, width, height);
                    double PHI, THETA;
                    sh::ToSphericalCoords(dir.cast<double>().normalized(), &PHI, &THETA);
                    ...
                    for (int l = 0; l <= SHOrder; l++)
                    {
                        for (int m = -l; m <= l; m++)
                        {
                            double basis = sh::EvalSH(l, m, PHI, THETA);
                            int _index = sh::GetIndex(l, m);
                            SHCoeffiecents[_index] += (Le * (float)basis * delta_PHI);
                        }
                    }
                }
            }
...
public:
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        for (int i = 0; i < mesh->getVertexCount(); i++)
        {
            ...
            auto shFunc = [&](double phi, double theta) -> double
            {
                ...
                else
                {
                    ...
                    double transport_sh = wi.dot(n);
                    if (transport_sh > 0) {
                        const Ray3f detect_ray(v, wi);
        ...
        if (m_Type == Type::Interreflection)
        {
            ...
            for (int count = 0; count < m_Bounce; count++) // for each bounce
            {
                for (int i = 0; i < mesh->getVertexCount(); i++)
                {
                    const Point3f &v = mesh->getVertexPositions().col(i);
                        ...
                        for (int p = 0; p < sampler_side; p++)
                        {
                            ...
                            if (transport_sh > 0)
                            {
                                ...
                                if(scene->rayIntersect(primary_ray)) // find the secondary shading point
                                {
                                    ...
                                    // and add up by barycoords as the SHCoeffs of the secondary shading point

                                    for (int k = 0; k < SHCoeffLength; k++)
                                    {
                                        const Vector3f tmp(m_TransportSHCoeffs.col((int)its.tri_index.x()).coeffRef(k),
                                                           m_TransportSHCoeffs.col((int)its.tri_index.y()).coeffRef(k),
                                                           m_TransportSHCoeffs.col((int)its.tri_index.z()).coeffRef(k)
                                        );

                                        coeffs[k] += tmp.dot(its.bary); 
                                    }
                                }
                            }
                        }
                    ...
                    // fill the intr_TransportSHCoeffs
                    for (int j = 0; j < SHCoeffLength; j++)
                    {
                        intr_TransportSHCoeffs.col(i).coeffRef(j) = coeffs[j] / m_SampleCount;
                    }
                }

                m_TransportSHCoeffs += intr_TransportSHCoeffs;
            }
        }
