...
namespace ProjEnv
{
                                                    ...
                                                    const int &channel)
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    Eigen::Vector3f dir = (faceDirX * u + faceDirY * v + faceDirZ).normalized();    // build dir for all pixels on cubemap
        ...
        float sumWeight = 0;
        for (int i = 0; i < 6; i++) // for each face of the cubemap
        {
            for (int y = 0; y < height; y++) // for each pixel on the face
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    for (int l = 0; l <= SHOrder; l++)
                    {
                        for (int m = -l; m <= l; m++)
                        {
                            float basis = sh::EvalSH(l, m, PHI, THETA);
                            int _index = sh::GetIndex(l, m);
                            SHCoeffiecents[_index] += (Le * basis * delta_PHI);
                        }
                    }
                }
            }
